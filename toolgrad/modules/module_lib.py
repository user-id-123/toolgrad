...
def get_success_execution_id(
    intermediate_steps: list[Tuple[ToolAgentAction, dict]]) -> int | None:
...
class ApiUseAgentModules:

  def __init__(self, seed: int = 1234, num_apis: int = 5):
    self.seed = seed
  ...
  def sample_apis(
    ...
    if seed is None:
      seed = self.seed
    if multi_thread:
      apis = toolbench_langchain_utils.get_all_apis(self.num_apis)
    else:
      raise NotImplementedError("Single-thread is not implemented yet.")
    self.apis = apis

  def get_all_tool_annotations(self):
    ...
    return {tool.name: get_one_tool_annotation(tool) for tool in self.apis}

  def get_literal_api_proposals_pydantic_model(self):
    ...
    apiuse_chainnode_customized = states.create_dedicated_apiusechainnode(
        allowed_values=[api.name for api in self.apis])
  ...
  def create_api_proposer(
      self, output_schema: Optional[BaseModel] = states.ApiProposalAll):
    llm = langchain.create_llm()
    self.sample_apis()
    json_schema = json.dumps(states.ApiProposalAll.model_json_schema())
    json_schema_str = f"```json\n{json_schema.replace('{', '{{').replace('}', '}}')}\n```"
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a helpful assistant."),
        ("human",
         prompt_lib.TOOLUSE_PROPOSER.format(
             workflow_cur="{workflow_cur}",
             api_proposals="{api_proposals}",
             schema_incontext=prompt_lib.SCHEMA_INCONTEXT_EXAMPLE,
             json_schema=json_schema_str,
         )),
        ("placeholder", "{agent_scratchpad}"),
    ])

    agent = create_tool_calling_agent(llm, self.apis, prompt)
  ...
  def create_api_executor(
    ...
    agent = toolbench_langchain_utils.create_tool_calling_agent(
        llm, apis_in_one_proposal, prompt)
    agent_executor = AgentExecutor(
        agent=agent,
  ...
  def create_api_execution_validator(self, key_list: List[str]):
    llm = langchain.create_llm()
  ...
  def create_api_selector(
      self,
      output_schema: BaseModel = states.ApiSelection) -> states.ApiSelection:
    llm = langchain.create_llm()
    api_selector = prompt_lib.SELECTOR | llm.with_structured_output(
        output_schema)
    return api_selector

  def create_workflow_updater(
      self,
      output_schema: BaseModel = states.ApiUseWorkflow,
  ):
  ...
  def structure_output(
      ...
      prompt_template: ChatPromptTemplate = prompt_lib.STRUCTURER_TEMPLATE,
  ):
    llm = langchain.create_llm()
    structurer = prompt_template | llm.with_structured_output(output_schema,)
    for msg in messages.messages:
      logging.debug(msg.pretty_repr())
    structured_output = structurer.invoke({"messages": messages})
...
async def single_api_execute_step(
  ...
  except (ValidationError, httpx.ReadTimeout, httpx.ConnectTimeout) as e:
    logging.info(
  ...
  if 'intermediate_steps' not in api_executions:
    logging.info(
        " ==== No intermediate steps found in the API execution result ==== ")
    return failure_report()
  processed_steps = [
      toolbench_langchain_utils.process_one_intermediate_step(
          inter_step, remove_msglog=True, hash_to_name=True)
  ...
  if last_success_step_id is None:
    logging.warning(f"API execution fails.")
    return failure_report()
